...
export class ScrollNavigator extends EleganceModule {
    constructor(themeConfig = {}) {
        super(MODULES.SCROLL_NAVIGATOR, themeConfig);
    ...
    #registerContainers() {
        document
            .querySelectorAll(`${this.config.containerSelector}[data-scroll-container-id]`)
            .forEach(element => this.registerContainer(element));
    }
    ...
    registerContainer(element) {
        ...
        const selector = this.config.slideSelector;
        const slides = Array.from(element.querySelectorAll(selector));

        if (slides.length === 0) {
            this.logger.warn(`No sections found in vertical container:`, element);
            return;
        }
        ...
        this.logger.log(`Registered vertical container with ${slides.length} sections`, scrollContainer);
    }

    #findParentContainer(element) {        
        return element.parentElement
            ?.closest(`${this.config.containerSelector}`);
    }
...
class ScrollContainer {
    constructor({ element, parent, slides }) {
        const defaultConfig = {
        ...
        this.config = this.#mergeConfig(defaultConfig, element.dataset);
    ...
    #mergeConfig(defaultConfig, dataset) {
        ...
        this.#configureDefaultPosition(config);

        return config;
    }

    #configureDefaultPosition(config) {
        let navPosition = config['navPosition'];                
        if (navPosition === undefined || navPosition === null) {
            navPosition = SCROLL_NAVIGATOR.NAV_POSITION.LEFT;
        }        
        const verticalPositions = [SCROLL_NAVIGATOR.NAV_POSITION.LEFT, SCROLL_NAVIGATOR.NAV_POSITION.RIGHT];        
        const isValidVertical = verticalPositions.includes(navPosition);        

        if (!isValidVertical) {
            const defaultPosition = SCROLL_NAVIGATOR.NAV_POSITION.LEFT;
            this.logger.log(`Assigned navigation elements position ${navPosition} is not valid for vertical direction! Using default (${defaultPosition}) instead`);
